...
import com.maxsella.fatmuscle.entity.PointArrayInfo;
import com.maxsella.fatmuscle.sdk.fat.entity.FindContourInfo;
...
//                }
    ...
    private int[] calArea(ArrayList<MatOfPoint> arrayList, Mat mat, int i, int i2) {
        ...
        Log.i("OpenCvMeasureUtil","Imgproc Contours star findSize:" + size);
        double d3 = -10000.0d;
        int i17 = -3;
        int i18 = 0;
        int i19 = -1;
        ...
        while (true) {
            ...
            if (i18 < arrayList.size()) {
                ...
                Log.i("OpenCvMeasureUtil","ImgprocoutLength:" + abs + " y:" + boundingRect.y + " rect.width=" + boundingRect.width + " rect.height=" + boundingRect.height + " contourArea:" + contourArea + " binValue=" + i);
                ...
                if (boundingRect.width < i2 || boundingRect.y < 10 || ((boundingRect.y < 10 && boundingRect.height <= 100) || boundingRect.height <= 2 || contourArea <= 10.0d)) {
                    ...
                    if (this.bodyPosition == 6) {
                        ...
                        if ((boundingRect.y + boundingRect.height < this.startPoint.y || boundingRect.y > this.endPoint.y) && i >= 50) {
                            Log.i("OpenCvMeasureUtil","Imgproc ==不在匹配范围内");
                            d3 = d4;
                            i19 = i25;
                            i22 = i27;
                            i20 = i9;
                    ...
                    if (boundingRect.width >= i2 && boundingRect.height > 20 && Math.abs(abs) <= 10 && Math.abs((boundingRect.width * boundingRect.height) - contourArea) < 400.0d) {
                        i21++;
                        Log.i("OpenCvMeasureUtil","Imgproc出现干净的直线，存在空打耦合的可能");
                        if (i21 > 1) {
                            return new int[]{-2};
                        }
                    ...
                    if ((boundingRect.height >= 200 || (boundingRect.height > 150 && maxValue2 < 100)) && contourArea > 10000.0d) {
                        if (this.bodyPosition == 6) {
                            i10 = 1;
                            Log.i("OpenCvMeasureUtil","ImgprocisAddTh==" + i10 + " slope=" + maxValue2 + " isFirst=" + this.isFirst);
                            if (abs <= 400 && boundingRect.height > 350 && ((i > 50 || !this.isFirst) && Math.abs((boundingRect.height * boundingRect.width) - contourArea) > boundingRect.height * 20)) {
                                ...
                                int i29 = i9;
                                Log.i("OpenCvMeasureUtil","ImgprocPASS weight=" + d2 + " selCount=" + i29 + " index=" + i18 + str + i + " 超出长度：" + d + " y=" + boundingRect.y + " slope =" + maxValue2);
                                if (d2 > d4) {
                                    d4 = d2;
                                    i25 = i18;
                                }
                    ...
                    int i292 = i9;
                    Log.i("OpenCvMeasureUtil","ImgprocPASS weight=" + d2 + " selCount=" + i292 + " index=" + i18 + str + i + " 超出长度：" + d + " y=" + boundingRect.y + " slope =" + maxValue2);
                    if (d2 > d4) {
                    }
                    i20 = i292 + 1;
                }
                ...
                if (i >= 50 || i31 != -2) {
                    if ((i32 < 2 || i >= 50) && (i32 < 3 || i >= 60)) {
                        ...
                        if (i34 != 6 || i32 <= 2 || i >= 150) {
                            if (i31 >= 0) {
                                ...
                                if (this.bodyPosition != 6) {
                                    ...
                                    if (boundingRect2.height > 80 && i > 90 && Math.abs((boundingRect2.height * boundingRect2.width) - contourArea2) < boundingRect2.height * 10) {
                                        Log.i("OpenCvMeasureUtil","Imgproc可能是空打耦合剂");
                                        return new int[]{-2};
                                    }
                                }
                                if (arcLength2 > 600 && boundingRect2.height > 300) {
                                    Log.i("OpenCvMeasureUtil","Imgproc图像太复杂，异常情况，不取");
                                    return new int[]{-2};
                                }
                                this.isFirst = false;
                                List<Point> list = arrayList.get(i31).toList();
                                Log.i("OpenCvMeasureUtil","Imgproc继续分析轮廓 size:" + list.size());
                                ...
                                Log.i("OpenCvMeasureUtil","Imgprocy的最大值 :" + algoDepth + " slope=" + i37 + " slopeB=" + maxValue3 + " maxX=" + i36);
                                ...
                                if (i38 == 6) {
                                    ...
                                    if (d6 >= this.startPoint.y) {
                                        ...
                                        if (d7 <= this.endPoint.y) {
                                            if (d6 < this.startGrayPoint.y || d7 > this.endGrayPoint.y) {
                                                Log.i("OpenCvMeasureUtil","Imgproc不在匹配的模板范围之内 startGrayPoint.y =" + this.startGrayPoint.y + " startGrayPoint.y=" + this.endGrayPoint.y);
                                                return i < 50 ? onBin(1 + i) : new int[]{-4};
                                            } else if (i33 > 3 && i30 > 15 && Math.abs(i37 - maxValue3) > 20 && boundingRect2.y > 200) {
                                                Log.i("OpenCvMeasureUtil","Imgproc多出很多半条线，图像复杂，不出图");
                                                return new int[]{-2};
                                            } else {
                                                c = 1;
                                            }
                                    ...
                                    Log.i("OpenCvMeasureUtil","Imgproc不在匹配的模板范围之内 startPoint.y =" + this.startPoint.y + " endPoint.y=" + this.endPoint.y);
                                ...
                                if (this.bodyPosition != 6) {
                                    ...
                                    Log.i("OpenCvMeasureUtil","Imgproc  x为0的Y值:" + i39 + " x为150的Y值=" + i40 + " 平均值=" + avgValue);
                                    if (i37 > 100 && i39 > avgValue && i40 > avgValue) {
                                        return new int[]{-1};
                                    }
                                }
                                Log.i("OpenCvMeasureUtil","Imgproc下边高度 slopeB = " + maxValue3 + " 上边高度slope=" + i37 + " 选中index=" + i5);
                                if (this.bodyPosition != 6) {
                                    if ((maxValue3 <= 20 && i37 > 20 && i < 120) || (maxValue3 < 30 && i37 > 30 && boundingRect2.height > 80)) {
                                        ...
                                        Log.i("OpenCvMeasureUtil","Imgproc上突，下平，可以适当添加阈值 slopeB = " + maxValue3 + " slope=" + i37 + " 加阈值后差异=" + compare);
                                        ...
                                        if (maxValue3 <= 20) {
                                            if (i37 > 50 && this.bodyPosition == 2) {
                                                Log.i("OpenCvMeasureUtil","Imgproc手臂 如果最后还是未能磨平，则不出图 ");
                                                return new int[]{-2};
                                            }
                                            i41 = 20;
                                        }
                                        if (maxValue3 <= i41 && i37 > 40 && this.bodyPosition != 2) {
                                            Log.i("OpenCvMeasureUtil","Imgproc如果最后还是未能磨平，则不出图 ");
                                            return new int[]{-2};
                                        }
                                    }
                                    i6 = i;
                                    ...
                                    if (maxValue3 <= 20 && i37 > 30 && i < 90) {
                                        Log.i("OpenCvMeasureUtil","Imgproc上突，下平，可以适当添加阈值 slopeB = " + maxValue3 + " slope=" + i37);
                                        i6 = i + 1;
                                ...
                                while (true) {
                                    ...
                                    int[] iArr2 = binArray;
                                    Log.i("OpenCvMeasureUtil","Imgproc compare = " + compare2 + " compareB=" + compare3 + " tempSlope=" + maxValue4 + " binValue=" + i6 + " findContourInfo=" + deelFindByBinValue.getFindCount() + " slopeB=" + maxValue + " starH=" + deelFindByBinValue.getStartHeight() + " endH=" + deelFindByBinValue.getEndHeight() + " maxX=" + deelFindByBinValue.getMaxX() + " maxValue=" + deelFindByBinValue.getMaxValue());
                                    ...
                                    if (i47 != 6) {
                                        ...
                                        if (compare2 > 0 && maxValue4 > i37 + 13) {
                                            Log.i("OpenCvMeasureUtil","Imgproc阈值已添加到极致，再添加则会变形，退出1");
                                            break;
                                        ...
                                        Log.i("OpenCvMeasureUtil","Imgproc退出2+preHeight=" + i42 + " findContourInfo.getHeight()=" + deelFindByBinValue.getHeight() + " preY=" + i43 + " findContourInfo.getY()=" + deelFindByBinValue.getY());
                                        break;
                                    } else {
                                        ...
                                        if (i49 >= 2) {
                                            Log.i("OpenCvMeasureUtil","Imgproc出现两种情况 " + deelFindByBinValue.getMaxX() + " v=" + deelFindByBinValue.getMaxValue());
                                            break;
                                ...
                                if (this.bodyPosition != 6) {
                                    ...
                                    if (deelFindByBinValue.getFindCount() >= 3 && i6 > 80) {
                                        Log.i("OpenCvMeasureUtil","Imgproc过虑到最后，如果还有三条以上比较完整的钱，则放弃查找 = ");
                                        return new int[]{-2};
                                    }
                                ...
                                if (this.queue.size() > 0) {
                                    ...
                                    Log.i("OpenCvMeasureUtil","Imgproc plugin resultAvg=" + avgValue2 + " size=" + this.queue.size() + " avg=" + size2);
                            ...
                            Log.i("OpenCvMeasureUtil","Imgproc 没有找到满足条件的一线条");
                            return new int[]{i23};
                        }
                        return onBin(i + 1);
                    }
